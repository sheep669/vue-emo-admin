                        ...
                        <el-form label-width="100px" class="emo-form-inline">
                            <el-form-item label="标签名">
                                <el-input
                                    style="width: 520px"
                                    clearable
                                    v-model="request_config.form.labelName"
                                    placeholder="请输入标签名"
                                ></el-input>
                            </el-form-item>
                        </el-form>
        ...
        >
            <el-form>
                ...
                <el-form-item label="标签名 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.labelName"
                        placeholder="请输入标签名"
                    ></el-input>
                </el-form-item>
                <el-form-item label="描述 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.description"
                        placeholder="请输入描述"
                    ></el-input>
                </el-form-item>
                <el-form-item
...
export default {
    ...
    methods: {
        ...
        doAddOrEdit() {
            //编辑模式
            if (this.eidtModel) {
                let url_param = constant.ml.updateUrl;
                ...
                //添加模式
                let url_param = constant.ml.addUrl;
                let data_param = this.dialogConfig;
        ...
        handleDelete(id) {
            console.log(id);
            doDeleteRequest(constant.ml.deleteUrl, id).then((res) => {
                console.log(res);
                if (res.data.code === 200) {
                    this.$message({
                        message: "删除成功",
        ...
        delBatch() {
            if (this.delIds.length == 0) {
                ...
                doPostRequest(constant.ml.deleteBatchUrl, this.delIds).then(
                    (res) => {
                        console.log(res);
                        if (res.data.code === 200) {
                            this.$message({
        ...
        serachData() {
            ...
            if (isInput) {
                searchOrGetRequest(
                    constant.ml.searchOrGetPageList,
                    page_parm,
                    data_param
                ).then((res) => {
                    if (res.data.code == 200) {
