        ...
        >
            <el-form>
                ...
                >
                    <el-input
                        clearable
                        v-model="dialogConfig.paySerialId"
                ...
                <el-form-item label="订单总额 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.totalOrderAmount"
                        placeholder="请输入订单总额(.00可省略) 保留两位小数 示例数据: 12 等同于 12.00"
                    ></el-input>
                </el-form-item>
                <el-form-item label="退款金额 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.refundAmount"
                ...
                <el-form-item label="实收金额 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.realAmount"
                        placeholder="请输入实收金额(.00可省略) 保留两位小数 示例数据: 12 等同于 12.00"
                    ></el-input>
                </el-form-item>
...
export default {
        ...
        return {
            ...
            table_config: {
                thead: [
                    ...
                    { label: "团长店铺", prop: "storeName", width: 280 },
                    { label: "订单总额", prop: "totalOrderAmount", width: 100 },
                    {
                        label: "退款金额",
                        prop: "refundAmount",
                        width: 100,
                    },
                    { label: "实收金额", prop: "realAmount", width: 100 },
                    {
                        label: "操作",
                        width: 200,
                        type: "slot",
    ...
    methods: {
        ...
        doAddOrEdit() {
            //编辑模式
            if (this.eidtModel) {
                let url_param = constant.ds.updateUrl;
                ...
                //添加模式
                let url_param = constant.ds.addUrl;
                let data_param = this.dialogConfig;
        ...
        handleEdit(id, data) {
            ...
            this.dialogFormVisible = true;
            this.dialogTitle = "编辑交易流水信息";
        },
        handleDelete(id) {
            console.log(id);
            doDeleteRequest(constant.ds.deleteUrl, id).then((res) => {
                console.log(res);
                if (res.data.code === 200) {
                    this.$message({
                        message: "删除成功",
        ...
        delBatch() {
            if (this.delIds.length == 0) {
                ...
                doPostRequest(constant.ds.deleteBatchUrl, this.delIds).then(
                    (res) => {
                        console.log(res);
                        if (res.data.code === 200) {
                            this.$message({
