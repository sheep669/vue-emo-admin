 ...
 * @Author: sheep669
 * @Description: 配送单管理模块
 * @Date: 2022-7-6 20:54:01
-->
<template>
    <!-- suggest to replace '_'===> '-' -->
    <div class="delivery_paper_manage">
                    ...
                    <el-col :span="14">
                        <el-form label-width="100px" class="emo-form-inline">
                            ...
                            <el-form-item label="仓库名称">
                                <el-input
                                    clearable
                                    v-model="request_config.form.depotName"
                                    placeholder="请输入"
                                ></el-input>
                            </el-form-item>
                            <el-form-item label="店铺地址">
                                <el-input
                                    clearable
                                    v-model="request_config.form.storeAddress"
                                    placeholder="请输入"
                                ></el-input>
                            </el-form-item>
                        </el-form>
        ...
        >
            <el-form>
                ...
                <el-form-item label="仓库名称 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.depotName"
                        placeholder="请输入仓库名称"
                    ></el-input>
                </el-form-item>
                ...
                <el-form-item label="配送路线 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.deliveryRoutes"
                        placeholder="请输入配送路线"
                    ></el-input>
                </el-form-item>
                <el-form-item label="商品总数 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.totalGoodsNumber"
                        placeholder="请输入商品总数"
                    ></el-input>
                </el-form-item>
...
export default {
        ...
        return {
            ...
            table_config: {
                thead: [
                    ...
                    { label: "仓库名称", prop: "depotName", width: 200 },
                    {
                        label: "店铺地址",
                        prop: "storeAddress",
                        width: 100,
                    },
                    { label: "配送路线", prop: "deliveryRoutes", width: 200 },
                    { label: "配送单生成时间", prop: "makeTime", width: 200 },
                    {
                        label: "配送状态",
                        prop: "deliveryStatus",
                        type: "function",
                        callback: (row, prop) => {
                            const data = this.delivery_status.filter(
                    ...
                    { label: "配送时间", prop: "deliveryTime", width: 200 },
                    { label: "签收时间", prop: "signTime", width: 200 },
                    { label: "商品总数", prop: "totalGoodsNumber", width: 100 },
                    {
                        label: "操作",
                        width: 200,
                        type: "slot",
    ...
    computed: {
        ...mapGetters(["delIds"]),
        ...mapState("status", ["delivery_status"]),
    },
    created() {
        this.getTableData();
    },
    ...
    methods: {
        ...
        doAddOrEdit() {
            //编辑模式
            if (this.eidtModel) {
                let url_param = constant.dpm.updateUrl;
                let data_param = this.dialogConfig;
                console.log(data_param);
                data_param.deliveryStatus = this.value;
                ...
                //添加模式
                let url_param = constant.dpm.addUrl;
                let data_param = this.dialogConfig;
        ...
        handleAdd() {
            ...
            this.dialogTitle = "添加配送单";
        },
        handleEdit(id, data) {
            console.log(id, data);
            this.eidtModel = true;
            this.dialogConfig = data;
            this.value = parseInt(data.deliveryStatus);
            this.dialogFormVisible = true;
            this.dialogTitle = "编辑配送单信息";
        },
        handleDelete(id) {
            console.log(id);
            doDeleteRequest(constant.dpm.deleteUrl, id).then((res) => {
                console.log(res);
                if (res.data.code === 200) {
                    this.$message({
                        message: "删除成功",
        ...
        delBatch() {
            if (this.delIds.length == 0) {
                ...
                doPostRequest(constant.dpm.deleteBatchUrl, this.delIds).then(
                    (res) => {
                        console.log(res);
                        if (res.data.code === 200) {
                            this.$message({
        ...
        serachData() {
            ...
            if (isInput) {
                searchOrGetRequest(
                    constant.dpm.searchOrGetPageList,
                    page_parm,
                    data_param
                ).then((res) => {
                    if (res.data.code == 200) {
