        ...
        >
            <el-form>
                ...
                <el-form-item label="标签编码 :" :label-width="formLabelWidth">
                    <el-input
                        clearable
                        v-model="dialogConfig.commodityCode"
                        placeholder="请输入标签编码"
                    ></el-input>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        doAddOrEdit() {
            //编辑模式
            if (this.eidtModel) {
                let url_param = constant.gl.updateUrl;
                ...
                //添加模式
                let url_param = constant.gl.addUrl;
                let data_param = this.dialogConfig;
        ...
        handleEdit(id, data) {
            ...
            this.dialogFormVisible = true;
            this.dialogTitle = "编辑商品标签信息";
        },
        handleDelete(id) {
            console.log(id);
            doDeleteRequest(constant.gl.deleteUrl, id).then((res) => {
                console.log(res);
                if (res.data.code === 200) {
                    this.$message({
                        message: "删除成功",
        ...
        delBatch() {
            if (this.delIds.length == 0) {
                ...
                doPostRequest(constant.gl.deleteBatchUrl, this.delIds).then(
                    (res) => {
                        console.log(res);
                        if (res.data.code === 200) {
                            this.$message({
